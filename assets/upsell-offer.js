...
window.__upsellSubmit = async function(form, evt) {
  // блокуємо нативний сабміт НАПЕВНЕ
  ...
  try {
    // 1) додаємо товар у кошик (як у твоєму робочому коді)
    const fd = new FormData(form);
    fd.append('sections_url', window.location.pathname);
    ...
    // 2) тягнемо секції і замінюємо дравер + bubble
    const sectionsJson = await fetchSections(SECTIONS);
    const drawerReplaced = replaceCartDrawer(sectionsJson);
    replaceBubble(sectionsJson);
  ...
  // критично: повертаємо false, щоб ЗАВЖДИ відмінити нативний сабміт
  return false;
};
